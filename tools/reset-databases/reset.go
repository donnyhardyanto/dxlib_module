	...
	if deleteAndCreateDb && len(config.Databases) > 0 {
		...
		configuration.Manager.NewIfNotExistConfiguration("storage", "storage.json", "json", false, false, map[string]any{
			adminDBNameId: map[string]any{
				...
				// Admin DB connection is INFRA: Vault -> env (e.g. DB_POSTGRES_ADDRESS from
				// run.env/key.env) -> default. Lets the maintenance DB address live with the
				// deploy instead of Vault. Requires dxlib >= the Get*OrEnvOrDefault release.
